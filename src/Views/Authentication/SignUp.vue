    ...
    <div class="absolute inset-0 z-0">
      <transition-group name="fade" mode="out-in">
        <div
          ...
          :style="{ backgroundImage: `url(/public/images/ground6.png)` }"
        ></div>
      </transition-group>
      <!-- Overlay -->
...
const handleSignUp = async () => {
  ...
  if (result.success) {
    toast.success('We emailed you a verification link. Please check your inbox.');
...
const handleGoogleSignUp = async () => {
  ...
  try {
    ...
    if (result.success) {
      toast.success('Sign up successful! Welcome to PYRAMIS.');
